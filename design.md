...
本文件是当前 Nuxt 工程 `/commerce/*` 的实现规范，来源为 `docs/reference/commerce-lens` 全部文档及八张原型截图。正式实现沿用原型的明亮工作台风格，路由使用独立 `commerce` 布局，样式统一以 `cl-` 前缀隔离。

## 实现约定

- 七个正式页面：经营总览、诊断工作台、异常中心、指标字典、数据中心、报告中心、运行审计；登录页 `/commerce/login`。
- 业务数字、指标版本、店铺列表、水位、计划、运行事件全部取自 `/api/commerce/v1`。没有数据时显示空态，不加载原型 fixture 或模拟定时器。
- 后端 `Cell.value` 为分或比率的十进制字符串，文本通过字符串/BigInt 格式化；图表只做经精度范围检查的坐标转换。变化、汇总、贡献使用服务端返回字段。
- 完整日默认来自最新已发布数据版本；全局筛选通过“应用范围”一次提交，历史运行与报告始终展示原范围。
- 共享基础类：`cl-card`、`cl-button`（`primary`/`soft`/`danger`）、`cl-badge`（`success`/`warning`/`danger`）、`cl-banner`、`cl-table`、`cl-field`、`cl-page-heading`、`cl-grid`、`cl-empty`。共享图标组件 `CommerceIcon` 使用轻量线性 SVG。
- 普通 API 状态使用骨架、内联错误与重试；不使用原型的“界面状态模拟器”。合成数据标签来自数据版本的 `synthetic`；本地确定性规划与真实模型规划明确区分。
- 报告用 Vue 文本模板和结构化结论卡渲染。证据通过独立 API 读取，抽屉四标签支持 Escape、Tab 焦点圈定和关闭后焦点回归。
- 错误文案描述可行动原因；长运行只显示已完成阶段，不伪造百分比。未知 Token 显示“未提供”。
...
参考原型位于 `docs/reference/commerce-lens/prototype`；正式样式位于 `data-agent-frontend-nuxt/app/assets/css/commerce.css`，代码使用 `--cl-*` Token，并保留上表短名称作为组件别名。总览首层依次展示支付 GMV、净收款额、支付订单数和退款金额。

正式页面的数据与状态来自真实接口，合成快照明确标记“合成数据”。范围筛选同时影响卡片、趋势、店铺表和异常摘要，历史报告单独标示固定范围。通过实际错误响应、权限回收与事件断线验证失败状态；不在正式页面提供原型状态模拟器。
