...
public class CommercePolicy {
    ...
    public CommerceSubject subject(String tenant, String subjectId) {
        ...
        return new CommerceSubject(tenant,subjectId,strings(member.path("storeIds")),strings(member.path("roles")),member.path("authzVersion").asLong());
    }
    public void assertCurrent(CommerceSubject subject) {
        CommerceSubject current=subject(subject.tenantId(),subject.subjectId());
